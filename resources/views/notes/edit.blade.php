    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6">
                ...
                <p class="mt-1 text-gray-400">Ubah detail catatan sesuai kebutuhan Anda</p>
            </div>

            <div class="bg-black border border-gray-800 rounded-xl overflow-hidden shadow-xl shadow-indigo-500/10">
                <form action="{{ route('notes.update', $note) }}" method="POST" class="relative">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="group">
                            ...
                            <textarea name="content" id="content" rows="6" 
                                class="w-full px-4 py-3 bg-gray-900/50 border border-gray-800 rounded-xl text-white placeholder-gray-500
                                       focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50
                                       transition-all duration-200 hover:bg-gray-900/75"
                                placeholder="Masukkan isi catatan">{{ $note->content }}</textarea>
                        </div>

                        <!-- Important Toggle -->
                        <div class="flex items-center">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_important" class="sr-only peer" value="1" {{ $note->is_important ? 'checked' : '' }}>
